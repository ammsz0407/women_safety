...
class _LiveSafeState extends State<LiveSafe> {
  List<Widget> policeStations = [];
  List<Widget> hospitals = [];
  double shakeThreshold = 15.0; // Adjust as needed for sensitivity
  ...
  // Send alert message and location
  Future<void> _sendAlert() async {
    try {
      Position position = await _getCurrentLocation();
      String message =
          "Emergency! I need help. My current location: https://maps.google.com/?q=${position.latitude},${position.longitude}";

      // Replace with the desired action (e.g., SMS, email, etc.)
      Fluttertoast.showToast(msg: "Alert sent: $message");
    } catch (e) {
  ...
  Future<void> findNearbyPoliceStations(Position position) async {
    String url =
        'https://overpass-api.de/api/interpreter?data=[out:json];node(around:5000,${position.latitude},${position.longitude})[amenity=police];out;';
  ...
  Future<void> findNearbyHospitals(Position position) async {
    String url =
        'https://overpass-api.de/api/interpreter?data=[out:json];node(around:5000,${position.latitude},${position.longitude})[amenity=hospital];out;';
    // Same implementation as your existing function
  }
